...
final mainColorProvider =
    StateNotifierProvider<MainColorNotifier, Color>(
...
// there's no backend "appearance"/color field, and every other
// appearance-like setting in this app (language, theme mode) already lives
// device-local, so this follows suit rather than inventing a second,
// conflicting settings source.
class MainColorNotifier extends StateNotifier<Color> {
  ...
  Future<void> setMainColor(final Color color) async {
    state = color;
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt(_mainColorPreferenceKey, color.value);
  }
}
