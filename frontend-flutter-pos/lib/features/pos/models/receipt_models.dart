/// Models matching the backend ReceiptResponse DTO.
/// Used by the ReceiptsScreen to render a structured receipt view.

/// Full receipt response from the backend.
class ReceiptResponse {
  ...
  /// frozen at that moment, so it stays correct even if Settings' rate
...
class ReceiptLine {
  ...
  /// Sum of modifier price deltas baked into [unitPrice] — e.g. 0.50 for a
  /// single "Large +$0.50" option. Zero when the line has no modifiers.
  final double modifierAmount;

  /// Human-readable modifier choices for this line, e.g.
  /// `"Sugar Level: 50%"` — set only when the sale line had any.
  final String? modifierSummary;

  /// [unitPrice] with [modifierAmount] backed out — the plain product price.
