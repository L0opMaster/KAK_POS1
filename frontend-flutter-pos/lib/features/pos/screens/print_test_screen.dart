...
//
// Nothing on this screen prints automatically; every job is a manual button
// press. See docs/PRINTING_SETUP.md for how to read the results.
...
class PrintTestScreen extends ConsumerStatefulWidget {
  const PrintTestScreen({super.key});

  @override
  ConsumerState<PrintTestScreen> createState() => _PrintTestScreenState();
}

class _PrintTestScreenState extends ConsumerState<PrintTestScreen> {
  String _log = '';
  bool _busy = false;
  ...
  /// Downloads the PDF directly (`Printing.sharePdf`, a browser file
  /// download on web) instead of opening the OS/Chrome print dialog
  /// (`Printing.layoutPdf`). Lets you inspect the generated file's actual
  /// page size/content directly — e.g. in a PDF viewer that reports page
  /// dimensions — with zero involvement from a printer driver, so a
  /// wrong-size printout can be attributed to either this app's PDF (this
  /// file is already wrong) or the Windows printer driver (this file is
  /// right, so the driver/paper config is the problem).
      ...
      body: ListView(
        ...
        children: [
          ...
          const Text(
            ...
            'generated the correct physical dimensions — independently of '
            'whichever printer/driver you print to.',
    ...
    return Table(
      ...
      children: [
        TableRow(
          ...
          children: [
            headerCell('Paper'),
            headerCell('Physical width'),
            headerCell('L/R margin'),
            headerCell('Content width'),
            headerCell('ESC/POS raster'),
          ],
        ),
