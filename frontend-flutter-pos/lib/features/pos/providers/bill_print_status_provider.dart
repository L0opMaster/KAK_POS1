...
/// `held_tickets_dialog.dart`'s ticket cards), distinct from payment
...
/// field. A ticket id is cleared once its ticket is paid or cancelled (see
/// `HeldTicketNotifier.releaseTicketById`/`deleteTicket`) so entries don't
/// linger forever — though a stale leftover id is harmless, since backend
...
class BillPrintStatusNotifier extends StateNotifier<Set<int>> {
  ...
  Future<void> _persist() async {
    ...
    await prefs.setStringList(
        _key, state.map((int id) => id.toString()).toList());
  }
