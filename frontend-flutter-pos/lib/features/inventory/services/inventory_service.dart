...
abstract class InventoryService {
  ...
  Future<List<StockMovementEntry>> getMovements({int? productId, int? storeId});
...
class ApiInventoryService extends InventoryService {
  ...
  // ── Counts ──
  @override
  Future<InventoryCount> getCount(String snapshotDate, {int storeId = 1}) async {
    final resp = await _api.get<List<dynamic>>(
  ...
  CountStatus _resolveCountStatus(List<InventoryCountItem> items) {
    final statuses =
        items.map((e) => (e.countStatus ?? 'SNAPSHOT').toUpperCase()).toSet();
  ...
  Future<PurchaseOrder> getPurchaseOrder(int id) async {
    final resp =
        await _api.get<Map<String, dynamic>>('/api/purchase-orders/$id');
  ...
  Future<PurchaseOrder> transitionPurchaseOrder(int id, String action) async {
    final resp = await _api
        .post<Map<String, dynamic>>('/api/purchase-orders/$id/$action');
    return PurchaseOrder.fromJson(resp);
  }
  ...
  Future<TransferOrder> completeTransferOrder(int id) async {
    final resp = await _api
        .post<Map<String, dynamic>>('/api/inventory/transfers/$id/complete');
    return TransferOrder.fromJson(resp);
  }
  ...
  Future<TransferOrder> cancelTransferOrder(int id) async {
    final resp = await _api
        .post<Map<String, dynamic>>('/api/inventory/transfers/$id/cancel');
    return TransferOrder.fromJson(resp);
  }
