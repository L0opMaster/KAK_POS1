...
/// by rank — and capped at the first 3 slots for all-pairs contexts (pie).
...
Widget _emptyState(BuildContext context, double height) => SizedBox(
      ...
      child: Center(
        child: Text(context.l10n.reportsNoDataForPeriod,
            style:
                const TextStyle(color: PosTheme.textHint, fontSize: 13)),
      ),
    );
...
/// series (e.g. top categories/items/employees by revenue).
...
class ReportLineChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double labelInterval =
        (data.length / 5).ceil().clamp(1, data.length).toDouble();
          ...
          lineBarsData: [
            LineChartBarData(
              ...
              belowBarData: BarAreaData(
                show: true,
                color: kReportChartPrimary.withOpacity(0.08),
              ),
            ),
          ],
        ),
...
/// Thin wrapper around fl_chart's [PieChart] with a side legend (identity is
/// never color-alone) for a multi-series share breakdown (e.g. payment mix).
