// Regression coverage for a bug fixed alongside the ReceiptLabels work:
// EscPosReceiptBuilder._buildLatinText only ever checked discountAmount and
// taxAmount directly, so a receipt with a delivery charge or an "other"
// service charge silently never printed that row on thermal output — even
// though the same receipt showed it correctly on screen and in the PDF.
// The fix iterates the shared `receipt.adjustments` list (the same list the
// on-screen preview and PDF already used), so this asserts every non-zero
// adjustment type actually reaches the printed byte stream, in the
// discount/delivery/other/tax order ReceiptViewModel.adjustments defines.
import 'dart:convert';
...
void main() {
  testWidgets(
    ...
    // EscPosReceiptBuilder.build loads a real asset (capabilities.json, via
    // CapabilityProfile.load) — that real I/O needs to escape testWidgets'
    // fake-async pump zone via runAsync, or the await never resolves.
    ...
    for (final label in [
      ...
      expect(text.contains(label), isTrue,
    ...
    expect(order, equals(order.toList()..sort()),
