// Ported from frontend-flutter-pos/test/khmer_receipt_dispatch_test.dart —
// PARTIAL PORT. The `EscPosReceiptBuilder dispatch` and `PrintService
// Khmer PDF embedding` groups are COPY/ADAPT NEARLY EXACTLY. Dropped: the
// `ThermalPrinterService.printReceipts` batch-dispatch group and
// `buildReceiptsPdf`'s mixed-batch test — both cover batch/"Print All"
// methods this port doesn't build (see thermal_printer_service.dart's and
// print_service.dart's doc comments for why).
//
// Regression coverage for "does the printing pipeline actually pick the
// image path for Khmer content, and only for Khmer content" — without
// depending on ReceiptBitmapRenderer's real off-screen widget rendering,
// which needs a live Overlay + frame pump and is covered by its own
// (narrower) test file. Both EscPosReceiptBuilder and PrintService accept
// an injectable ReceiptBitmapRenderer for exactly this reason: substitute
// a fake that returns a trivial image instantly, and assert on WHETHER it
// was called and what bytes/PDF structure resulted — proving the dispatch
// wiring (containsKhmer → bitmap; otherwise → native) without needing to
// also exercise real Khmer text shaping in this test.
import 'dart:convert';
...
void main() {
  ...
  group('EscPosReceiptBuilder dispatch', () {
    testWidgets(
      ...
      (tester) async {
        ...
        final bytes = await tester.runAsync(
          () => builder.build(ctx, receipt, PrinterPaperSize.mm80),
        );
        ...
        expect(
          latin1.decode(bytes, allowInvalid: true).contains('Thank you'),
          isTrue,
        );
      },
    );
    ...
    testWidgets(
      ...
      'still routes through the bitmap path',
      (tester) async {
        final ctx = await pumpContext(tester);
        final fake = _FakeBitmapRenderer();
        final builder = EscPosReceiptBuilder(bitmapRenderer: fake);
  ...
  group('PrintService Khmer PDF embedding', () {
