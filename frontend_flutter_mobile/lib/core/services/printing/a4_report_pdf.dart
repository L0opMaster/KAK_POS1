// Ported from `frontend-flutter-pos/lib/core/services/printing/
// a4_report_pdf.dart` — COPY/ADAPT NEARLY EXACTLY, full file.
//
...
// English/numeric text is unaffected — it stays plain vector pw.Text.
...
class A4ReportPdf {
  ...
  /// text, this class just owns where in the footer they're placed.
  static Future<Uint8List> build({
    ...
    final theme = await timePrintStage(
      'a4FontLoad',
      () => KhmerPdfFont.loadTheme(),
    );
    ...
    final rasterized = await timePrintStage('a4RasterizationTotal', () async {
      final businessNameWidget = (businessName != null && businessName.isNotEmpty)
          ? await KhmerTextRasterizer.textOrImage(
              businessName,
              fontSize: 18,
              bold: true,
            )
          : null;
      final businessAddressWidget =
          (businessAddress != null && businessAddress.isNotEmpty)
          ? await KhmerTextRasterizer.textOrImage(
              businessAddress,
              fontSize: 10,
            )
          : null;
      final businessPhoneWidget = (businessPhone != null && businessPhone.isNotEmpty)
          ? await KhmerTextRasterizer.textOrImage(
              'Tel: $businessPhone',
              fontSize: 10,
            )
          : null;
      final titleWidget = await KhmerTextRasterizer.textOrImage(
        title,
        fontSize: 15,
        bold: true,
      );
      ...
      final pageLabelWidget = await KhmerTextRasterizer.textOrImage(
        pageLabel,
        fontSize: 8,
        color: PdfColors.grey600,
      );

      final headerCells = await Future.wait(
        columns.map(
          (c) => KhmerTextRasterizer.textOrImage(
            c,
            fontSize: 9.5,
            bold: true,
      ...
      final rowCells = await Future.wait(
        rows.map(
          (row) => Future.wait(
      ...
      final summaryWidgets = await Future.wait(
        summary.map((e) async {
          final key = await KhmerTextRasterizer.textOrImage(
            '${e.key}: ',
            fontSize: 11,
          );
          final value = await KhmerTextRasterizer.textOrImage(
            e.value,
            fontSize: 11,
            bold: true,
          );
      ...
      final detailWidgets = await Future.wait(
        details.map((e) async {
          final key = await KhmerTextRasterizer.textOrImage(
            '${e.key}: ',
            fontSize: 10,
