...
/// khmer_text_rasterizer.dart` — COPY/ADAPT NEARLY EXACTLY, full file.
///
...
/// (see khmer_pdf_font.dart's doc comment for why).
///
/// Unlike [ReceiptBitmapRenderer] (which rasterizes a WHOLE receipt
/// document by mounting a real widget tree in an Overlay), this
/// rasterizes a single run of text with no widget tree/BuildContext/
/// Overlay needed at all — TextPainter lays out and paints directly onto
/// a canvas. That distinction matters here specifically: an A4 report's
/// table borders, colors and row striping must stay real PDF vector
/// drawing (only Khmer TEXT becomes a bitmap) — rasterizing the whole
/// document, the way receipts do, would throw away the table structure
/// and be far too slow for a 100+ row report.
...
class KhmerTextRasterizer {
  ...
  static Future<pw.Widget> textOrImage(
    ...
    final rasterized = await _cache.putIfAbsent(
      key,
      () => _rasterize(text, fontSize, bold, color),
    );
    ...
    return timePrintStage('khmerTextRasterizeTotal', () async {
      ...
      if (byteData == null) {
        throw const KhmerTextRenderException(
          'Unable to render Khmer text: failed to encode the rendered '
          'image.',
        );
      }
