...
/// escpos_receipt_builder.dart` — COPY/ADAPT NEARLY EXACTLY, full file.
///
...
/// ESC/POS text; any receipt containing Khmer prints as a single
...
class EscPosReceiptBuilder {
  ...
  List<int> _buildLatinText(Generator generator, ReceiptViewModel receipt) {
    ...
    if (receipt.isBill && receipt.tableNumber != null) {
      line(
        receipt.tableNumber!.toUpperCase(),
    ...
    for (final l in receipt.lines) {
      line(l.name);
      row(
        '  ${l.qty.toStringAsFixed(0)} x ${receipt.fmt(l.unitPrice)}',
    ...
    // checking discountAmount/taxAmount directly would silently drop a
    // delivery or other/service charge from thermal output, unlike the
    // PDF/on-screen preview.
    ...
    if (receipt.isBill) {
      ...
      if (receipt.changeAmount > 0) {
        ...
        // — Paid alone is the amount APPLIED to the sale (never more than
        // the total), so Change would otherwise look like it appeared from
        // nowhere.
        row(
          labels.cashReceived,
