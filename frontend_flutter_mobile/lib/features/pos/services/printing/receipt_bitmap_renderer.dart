...
/// receipt_bitmap_renderer.dart` — COPY/ADAPT NEARLY EXACTLY, full file.
///
...
class ReceiptBitmapRenderer {
  ...
  /// non-receipt tickets (queue-number ticket — see `print_service.dart`)
  /// that need a single Khmer field rendered as a bitmap but have no
  /// [ReceiptViewModel] behind them to build one from.
  ///
  /// Requires a [BuildContext] with an [Overlay] ancestor (any screen
  /// context works — the app always has a root `Navigator`/`Overlay`).
  Future<img.Image> renderWidgetImage(
    ...
    // never accidentally pick up desktop/full-screen/A4 width. The
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint(
          '[ReceiptLayout] mode=raster paper=${paperSize.name} '
      ...
      // Raw RGBA, not PNG: a full zlib compress+decompress round trip for
      // bytes that would be thrown away anyway. Raw RGBA is an uncompressed
      // memory copy on both sides, and img.Image.fromBytes accepts it
      // directly — same pixels, no compression work.
      ...
      if (byteData == null) {
        throw const ReceiptRenderException(
          'Unable to render Khmer receipt: failed to encode the rendered '
          'image.',
        );
      }
