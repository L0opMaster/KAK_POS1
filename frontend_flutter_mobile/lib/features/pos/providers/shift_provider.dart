...
/// shift_provider.dart` — COPY/ADAPT NEARLY EXACTLY, including
/// `ShiftHistoryNotifier`/`ShiftHistoryState` (Day 10's "...History").
class ShiftNotifier extends StateNotifier<ShiftState> {
  ...
  /// Close the current shift. Sets `isShiftOpen: false` UNCONDITIONALLY,
  /// regardless of whether the backend returned `CLOSED` or
  /// `PENDING_APPROVAL` (a large-variance close by a non-owner/manager
  /// account) — callers must read `state.currentShift?.status`, not just
  /// `isShiftOpen`, to distinguish the two. The variance/approval threshold
  /// itself is computed entirely on the shared backend
  /// (`ShiftService.close`, `VARIANCE_THRESHOLD = $10.00`) — never
  /// reimplement it here.
...
class ShiftHistoryState {
  ...
  ShiftHistoryState({required this.loading, this.error, required this.shifts});
