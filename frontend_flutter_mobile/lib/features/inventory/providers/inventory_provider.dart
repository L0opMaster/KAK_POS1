...
/// inventory_provider.dart` — COPY/ADAPT NEARLY EXACTLY, full file.
...
// differently.)
// ─────────────────────────────────────────────────────────────
final movementsProvider = StateNotifierProvider<
    MovementsNotifier, AsyncValue<List<StockMovementEntry>>>(
  (ref) {
...
class MovementsNotifier
  ...
  Future<void> load({int? productId, int? storeId}) async {
    try {
      ...
      final list = await _service.getMovements(
        productId: productId,
        storeId: storeId,
      );
...
// ─────────────────────────────────────────────────────────────
final inventoryCountProvider = StateNotifierProvider<InventoryCountNotifier,
...
class InventoryCountNotifier
    extends StateNotifier<AsyncValue<InventoryCount?>> {
  final InventoryService _service;
...
class InventoryValuationNotifier
  ...
  InventoryValuationNotifier(this._service)
