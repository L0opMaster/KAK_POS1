/// Ported from `frontend-flutter-pos/lib/features/inventory/models/
/// inventory_models.dart` — COPY/ADAPT NEARLY EXACTLY, full file.
///
...
class PurchaseOrder {
  ...
  Map<String, dynamic> toJson() => {
    ...
    if (storeId != null) 'storeId': storeId,
    if (orderDeadline != null) 'orderDeadline': _dateOnly(orderDeadline!),
      ...
      InventoryValuationReport(
        ...
        items:
            (json['items'] as List<dynamic>?)
                ?.map(
                  (e) =>
                      InventoryValuationItem.fromJson(e as Map<String, dynamic>),
                )
                .toList() ??
            [],
        totalValue: (json['totalValue'] as num?)?.toDouble() ?? 0,
...
class StoreLocation {
  ...
  StoreLocation({this.id, required this.name, this.address, this.active = true});
