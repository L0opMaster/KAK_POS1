...
/// Ported from `frontend-flutter-pos/lib/features/reports/services/
/// report_service.dart` — COPY/ADAPT NEARLY EXACTLY, full file, including
/// `fetchAllPages`/`reportPrintPageSize` (Day 17's inventory PDF exports,
/// deferred pending this file, can now be wired to the same mechanism).
abstract class ReportService {
  ...
  /// Top products
  Future<List<TopProduct>> topProducts({String? date, String? from, String? to});
...
class ApiReportService extends ReportService {
  ...
  Future<List<MonthlySales>> monthlySales(int year) async {
    ...
    return resp.cast<Map<String, dynamic>>().map((e) => MonthlySales.fromJson(e)).toList();
  }
  ...
  Future<List<TopProduct>> topProducts({
    ...
    return resp.cast<Map<String, dynamic>>().map((e) => TopProduct.fromJson(e)).toList();
  }
  ...
  Future<List<StockMovementRow>> stockMovements({
    ...
    return resp.cast<Map<String, dynamic>>().map((e) => StockMovementRow.fromJson(e)).toList();
  }
